...
namespace Lab_18
{
    class Program
    {
        public static void Main()
        {
            ...
            DriverOfBoat boatDriver = new(); //Adapter демонстрация
            ...
            Truckdriver.createDriver(10, false, 2022, 2, "Кирилл", 4);
            ...
            boatDriver.createDriver(300, true, 2020, 8, "Андрей", 0);
            boatDriver.createDriver(700, false, 2019, 1, "Серафим", 1);
            boatDriver.createDriver(30, true, 2018, 10, "Артем", 2);

            //ДЕМОНСТРАЦИЯ DECORATORa
            Client cl1 = new("Артём");
            var flight1 = cl1.CreateNewFlight("Бетонные плиты", 300, 31);
            Dispatcher.PerfomanceCheck(flight1);
            Console.ReadLine();
            UpDatesDrivers.MakeDriverWithMoreExperience(TypeOfCar.car, 2, 10);
            Dispatcher.PerfomanceCheck(flight1);
            //-----------------------------
            Console.ReadLine();

            //command демонстрация
            ControlDrivers control = new();
            Command cmd = new(control);
            cmd.sort();
            cmd.search("Андрей");
            //-------------------------------
            ...
            //Паттерн STATE
            Console.ReadLine();
            Dispatcher._CheckExperience(TypeOfCar.car, 4);
            //--------------------------------
            //Паттерн Memento
            Console.ReadLine();
            DriverHistory dr_History= new DriverHistory();
            dr_History.history.Push(Driver.dr[5].saveExp(5));
            UpDatesDrivers.MakeDriverWithMoreExperience(TypeOfCar.FreightCar, 5, 10);
            Driver.dr[5].RestoreState(dr_History.history.Pop(), 5);
            UpDatesDrivers.MakeDriverWithMoreExperience(TypeOfCar.FreightCar, 5, 2);
            //-------------------------
            #endregion
            #region ex2
            Console.ReadLine();
